    ...
    for my_miso_chan in chan_lst:
        ...
        for ibo_val in ibo_arr:
            ...
            for ebn0_step_val in ebn0_step:
                ...
                my_cnc_rx.update_distortion(ibo_db=ibo_db)

                abs_alpha = my_mod.calc_alpha(ibo_db=ibo_db)
                ber_per_dist = []
                    ...
                    for idx, snr in enumerate(snr_arr):
                        ...
                        while bits_sent < bits_sent_max and n_err < n_err_min:
                            ...
                            if run_idx == 0:
                                ...
                                rx_ofdm_symbol = my_noise.process(rx_ofdm_symbol, avg_sample_pow=my_mod.avg_symbol_power * np.mean(
                                    np.power(agc_corr_nsc, 2)) * abs_alpha ** 2)
                            # apply AGC
                ...
                for idx, cnc_iter_val in enumerate(cnc_n_iter_lst):
                    if idx == 0:
                        ...
                        ax1.plot(ebn0_arr, ber_per_dist[idx], label="CNC NI = %d" % (cnc_iter_val))

                # fix log scaling
                ax1.set_title("BER vs Eb/N0, %s, CNC, QAM %d, N ANT = %d, IBO = %d [dB]" % (my_miso_chan, my_mod.constellation_size, n_ant_val, ibo_db))
                ...
                filename_str = "ber_vs_ebn0_cnc_%s_nant%d_ibo%d_ebn0_min%d_max%d_step%1.2f_niter%s" % (my_miso_chan, n_ant_val, ibo_db, min(ebn0_arr), max(ebn0_arr), ebn0_arr[1]-ebn0_arr[0], '_'.join([str(val) for val in cnc_n_iter_lst[2:]]))
