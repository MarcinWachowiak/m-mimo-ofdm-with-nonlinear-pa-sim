            ...
            for ibo_val_db in ibo_arr:
                ...
                for ebn0_step_val in ebn0_step:
                    ...
                    for snr_idx, snr_db_val in enumerate(snr_arr):
                        ...
                        mp_link_obj = link.Link(mod_obj=my_mod, array_obj=my_array, std_rx_obj=my_standard_rx,
                        ...
                        proc_seed_lst = [[1, 2, 3]] * num_cores
                        processes = []
                        for idx in range(num_cores):
                            p = mp.Process(target=mp_link_obj.simulate, args=(
                            cnc_n_iter_lst, proc_seed_lst[idx], n_err_shared_arr, n_bits_sent_shared_arr))
                            processes.append(p)
                            p.start()
                        ...
                        #    pool.starmap(mp_link_obj.simulate, args=(cnc_n_iter_lst, proc_seed_lst[idx], n_err_shared_arr, n_bits_sent_shared_arr))

                        # mp_link_obj.simulate(cnc_n_iter_lst, n_err, bits_sent)
