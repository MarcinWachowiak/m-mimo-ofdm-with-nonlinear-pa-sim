...
if __name__ == '__main__':
    ...
    my_tx = transceiver.Transceiver(modem=copy.deepcopy(my_mod), impairment=copy.deepcopy(my_distortion),
                                    carrier_spacing=int(15e3), center_freq=int(3.5e9))
            ...
            while bits_sent < bits_sent_max and n_err < n_err_min:
                ...
                if include_clean_run and run_idx == 0:
                    ...
                    no_cp_fd_sig_mat = torch.fft.fft(torch.from_numpy(rx_ofdm_symbol[my_cnc_rx.modem.cp_len:]),
                                                     norm="ortho").numpy()
                    # TODO: update the script to use the new class methods
                    rx_bits = my_cnc_rx.receive(n_iters=cnc_n_iter_val, in_sig_fd=no_cp_fd_sig_mat)
