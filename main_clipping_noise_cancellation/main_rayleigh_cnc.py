    ...
    for idx, snr in enumerate(snr_arr):
        ...
        while bits_sent < bits_sent_max and n_err < n_err_min:
            ...
            if include_clean_run and run_idx == 0:
                rx_ofdm_symbol = my_noise.process(clean_rx_ofdm_symbol_fd, avg_sample_pow=my_mod.avg_sample_power * (
                        my_cnc_rx.modem.calc_alpha(ibo_val_db) ** 2))
            else:
                rx_ofdm_symbol = my_noise.process(rx_ofdm_symbol_fd, avg_sample_pow=my_mod.avg_sample_power * (
                        my_cnc_rx.modem.calc_alpha(ibo_val_db) ** 2))
