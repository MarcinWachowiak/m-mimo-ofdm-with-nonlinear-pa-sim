...
# fix log scaling
ax1.set_title("BER vs Eb/N0, MCNC, QAM %d, N ANT = %d, IBO = %d [dB]" % (my_mod.constellation_size, n_ant, ibo_db))
...
filename_str = "ber_vs_ebn0_mcnc_%s_nant%d_ibo%d_ebn0_min%d_max%d_step%1.2f_niter%s" % (my_miso_chan, n_ant, ibo_db, min(ebn0_arr), max(ebn0_arr), ebn0_arr[1]-ebn0_arr[0], '_'.join([str(val) for val in cnc_n_iters_lst[2:]]))
