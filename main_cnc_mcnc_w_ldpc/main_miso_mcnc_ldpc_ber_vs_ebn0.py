    ...
    for my_miso_chan in chan_lst:
        ...
        for ibo_val_db in ibo_arr:
            ...
            for ebn0_step_val in ebn0_step:
                ebn0_arr = np.arange(-2, 2.1, ebn0_step_val)
                ...
                for snr_idx, snr_db_val in enumerate(snr_arr):
                    ...
                    bers = np.zeros([len(mcnc_n_iter_lst) + 1])
                    n_err = np.zeros([len(mcnc_n_iter_lst) + 1])
                    bits_sent = np.zeros([len(mcnc_n_iter_lst) + 1])
                    # clean RX run
                    snap_cnt = 0
                    while True:
                        ...
                        if np.logical_and((n_err[0] < n_err_min), (bits_sent[0] < bits_sent_max)):
                            ...
                            ldpc_encoded_bits = np.squeeze(np.array(
                                matlab.nrLDPCEncode(matlab.int8(matlab.transpose(tx_bits)), base_graph_number)))

                            clean_ofdm_symbol = my_array.transmit(ldpc_encoded_bits, out_domain_fd=True,
                                                                  return_both=False,
                                                                  skip_dist=True)
                            ...
                            rx_bits = np.squeeze(np.array(matlab.transpose(
                                matlab.nrLDPCDecode(matlab.transpose(matlab.double(rx_llr_soft_bits)),
                                                    base_graph_number, max_ldpc_ite))))

                            # noise + dist variance per symbol = noise variance
                            n_bit_err = count_mismatched_bits(tx_bits, rx_bits)
                            n_err[0] += n_bit_err
                            bits_sent[0] += n_info_bits_per_block
                        else:
                            break
                        snap_cnt += 1
                    ...
                    while True:
                        ...
                        if ite_use_flags.any() == True:
                            curr_ite_lst = mcnc_n_iter_lst[ite_use_flags]
                        else:
                            break
                        ...
                        tx_bits = bit_rng.choice((0, 1), n_info_bits_per_block)
                        ldpc_encoded_bits = np.squeeze(
                            np.array(matlab.nrLDPCEncode(matlab.int8(matlab.transpose(tx_bits)), base_graph_number)))

                        tx_ofdm_symbol = my_array.transmit(ldpc_encoded_bits, out_domain_fd=True, skip_dist=False)
                        ...
                        rx_sybmols_per_iter_lst = my_mcnc_rx.receive(n_iters_lst=curr_ite_lst, in_sig_fd=rx_ofdm_symbol,
                                                                    return_bits=False, noise_var=noise_var_snr_based, matlab=matlab)
                        ...
                        for ite_idx in range(len(rx_sybmols_per_iter_lst)):
                            ...
                            bits_sent[act_ber_idx[ite_idx]] += n_info_bits_per_block
                        snap_cnt += 1
                ...
                for idx, mcnc_iter_val in enumerate(mcnc_n_iter_lst):
                    if idx == 0:
                        ax1.plot(ebn0_arr, ber_per_dist[idx + 1, :], label="Standard RX")
                    else:
                        ax1.plot(ebn0_arr, ber_per_dist[idx + 1, :], label="MCNC NI = %d" % (mcnc_iter_val))
                ...
                filename_str = "ldpc_in_loop_ber_vs_ebn0_mcnc_%s_nant%d_ibo%d_ebn0_min%d_max%d_step%1.2f_niter%s" % (
