...
matlab.qd_channel_env_setup(matlab.double(n_ant), matlab.double(n_sub_carr), matlab.double(subcarr_spacing), matlab.double(center_freq), matlab.double(distance), channel_model_str, nargout=0)
...
for idx in range(5):
    ...
    channel_mat = np.array(matlab.qd_get_channel_mat(distance, 0, 1.5))
