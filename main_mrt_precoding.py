...
for run_idx in range(2):
    ...
    my_rx = transceiver.Transceiver(modem=my_mod,  impairment=None, cord_x=0.2, cord_y=0.2)
    ...
    n_points = 360
    rx_points = pts_on_circum(r=0.2, n=n_points)
...
for res_idx in range(2):
    if res_idx==0:
        ax1.plot(radian_vals_lst[res_idx], psd_at_angle_lst[res_idx], label="Exact")
    else:
        ax1.plot(radian_vals_lst[res_idx], psd_at_angle_lst[res_idx], label="Approx")
...
ax1.plot(np.array(sorted_rx_at_point_freq_arr), to_db(np.array(sorted_psd_at_point_arr)), label="At angle: %d$\degree$" % point_idx_psd)
...
ax1.grid()
plt.tight_layout()
plt.savefig("figs/psd_at_angle_45_deg.png", dpi=600, bbox_inches='tight')
plt.show()
