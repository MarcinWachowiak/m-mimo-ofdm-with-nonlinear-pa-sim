"""
Measure the signal-to-distortion ratio as a function of the azimuth angle
for a uniform circular array, selected number of antennas and channels.
"""
...
if __name__ == '__main__':
    ...
    # %%
    plot_full_circle = True

    if plot_full_circle:
        n_points = 360
        rx_points = pts_on_circum(radius=300, n_points=n_points)
    else:
        n_points = 180
        rx_points = pts_on_semicircum(radius=300, n_points=n_points)
    ...
    ax3.set_xticks(np.linspace(0, 360, 13, endpoint=True))
    ax3.set_xlim([0, 360])
    ax3.set_ylim([23, 40])
    fig.suptitle("Signal to distortion ratio")

    plt.tight_layout()
    plt.savefig("../figs/uca_sdr_at_angle_ibo%d_%dto%dant_sweep.png" % (
