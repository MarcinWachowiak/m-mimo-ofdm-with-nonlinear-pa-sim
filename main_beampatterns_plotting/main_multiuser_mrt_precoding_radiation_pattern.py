...
# %%
my_mod = modulation.OfdmQamModem(constel_size=64, n_fft=4096, n_sub_carr=2048, cp_len=128, n_users=n_users)
...
for channel_type_str in channel_type_lst:
    ...
    for n_ant_val in n_ant_vec:
        ...
        psd_sel_filename_str = "multiuser_psd_mrt_%s_chan_ibo%d_npoints%d_nsnap%d_angle%d_nant%d" % (
        ...
        # %%
        filename_str = "multiuser_mrt_sig_powers_vs_angle_%s_chan_ibo%d_npoints%d_nsnap%d_angle%d_nant%d" % (
            my_miso_chan, ibo_val_db, n_points, n_snapshots, sel_psd_angle, n_ant_val)

        data_lst_per_nant = []
        tmp_total_lst = [desired_sig_power_per_nant, distortion_sig_power_per_nant]
