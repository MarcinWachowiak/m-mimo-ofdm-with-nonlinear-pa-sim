...
if __name__ == '__main__':
    ...
    my_toi_dist = distortion.ThirdOrderNonLin(toi_db=10.0, avg_samp_pow=my_mod.avg_sample_power)
    my_softlim_dist = distortion.SoftLimiter(ibo_db=0.0, avg_samp_pow=my_mod.avg_sample_power)

    my_distortion_obj_lst = [my_softlim_dist, my_toi_dist]

    desired_sig_pow_per_dist_obj = []
    distorted_sig_inband_pow_per_dist_obj = []
    distorted_sig_oob_pow_per_dist_obj = []
    distorted_sig_combined_pow_per_dist_obj = []

    for my_distortion in my_distortion_obj_lst:
            ...
            for my_miso_chan in chan_lst:
                ...
                for ibo_val_db in ibo_arr:
                    ...
                    if estimate_alpha:
                        ...
                        for usr_idx in range(n_users):
                            ...
                            while ofdm_symb_idx < n_ofdm_symb:
                                ...
                                rx_sig_fd = np.multiply(tx_ofdm_symbol_fd, usr_chan_mat_lst[usr_idx])
                                rx_sig_clean_fd = np.multiply(clean_ofdm_symbol_fd, usr_chan_mat_lst[usr_idx])

                                clean_nsc_ofdm_symb_fd = np.concatenate((rx_sig_clean_fd[:, -my_mod.n_sub_carr // 2:],
                                                                         rx_sig_clean_fd[:,
                                                                         1:(my_mod.n_sub_carr // 2) + 1]),
                                                                        axis=1)
                                ...
                                ofdm_symb_idx += 1
                                alpha_val_vec.append(
                                    np.abs(np.average(alpha_numerator_vec / alpha_denominator_vec, axis=1)))
                            # calculate alpha average
                            alpha_vec_est = np.average(alpha_val_vec, axis=0)
                            alpha_vec_per_usr.append(alpha_vec_est)

                        print("Alpha coeff estimate:", alpha_vec_per_usr)
                    ...
                    if plot_precoding_beampatterns:
                        ...
                        distorted_sig_combined_pow_per_pt = []
                        for pt_idx, point in enumerate(rx_points):
                            ...
                            distorted_sig_combined_pow_per_pt.append(
                                np.sum(distortion_inband_sig_pow_arr) + np.sum(distortion_oob_sig_pow_arr))

                        desired_sig_pow_per_dist_obj.append(desired_sig_pow_per_pt)
                        distorted_sig_inband_pow_per_dist_obj.append(distorted_sig_inband_pow_per_pt)
                        distorted_sig_oob_pow_per_dist_obj.append(distorted_sig_oob_pow_per_pt)
                        distorted_sig_combined_pow_per_dist_obj.append(distorted_sig_combined_pow_per_pt)
                        ...
                        if plot_psd:
                            ...
                            rx_dist_at_sel_point_freq_arr, rx_dist_at_sel_point_psd = welch(
                                rx_sig_at_sel_point_dist_arr,
                                fs=psd_nfft,
                                nfft=psd_nfft,
                                nperseg=n_samp_per_seg,
                                return_onesided=False)
                            ...
                            ax5.grid()
                            plt.tight_layout()
                            plt.savefig("../figs/multiuser/psd/%s.png" % psd_sel_filename_str, dpi=600,
                                        bbox_inches='tight')
                            plt.show()
                            # plt.cla()
                            # plt.close()
    ...
    for dist_idx, my_distortion in enumerate(my_distortion_obj_lst):
        ax1.plot(radian_vals, utilities.to_db(desired_sig_pow_per_dist_obj[dist_idx]),
                 label="%s Desired" % my_distortion,
                 linewidth=0.5)
        ax1.plot(radian_vals, utilities.to_db(distorted_sig_combined_pow_per_dist_obj[dist_idx]),
