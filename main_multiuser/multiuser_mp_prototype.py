"""
Multiprocess/parallel version of the simulation of the MCNC algorithm in a two-user scenario.
"""
...
def multiuser_mcnc_parallel(proc_idx, precoding_str, bits_sent_max, n_err_min, seeds_lst):
    data_lst = []

    set_latex_plot_style()
    CB_color_cycle = ['#006BA4', '#FF800E', '#ABABAB', '#595959', '#5F9ED1', '#C85200', '#898989', '#A2C8EC',
                      '#FFBC79',
                      '#CFCFCF']
    # Multiple users data
        ...
        for my_miso_chan in chan_lst:
            for ibo_val_db in ibo_arr:
                ...
                if estimate_alpha:
                    ...
                    bit_rng = np.random.default_rng(seeds_lst[1])
                    n_ofdm_symb = 1e2
                    ofdm_symb_idx = 0
                    alpha_val_vec = []
                ...
                loc_rng = np.random.default_rng(seeds_lst[0])
                if meas_usr_sdr:
                    print("Per user SDR")
                ...
                for ebn0_step_val in ebn0_step:
                    ...
                    bit_rng = np.random.default_rng(seeds_lst[1])
                    snr_arr = utilities.ebn0_to_snr(ebn0_arr, my_mod.n_sub_carr, my_mod.n_sub_carr,
                                                    my_mod.constel_size)
                    ...
                    for snr_idx, snr_db_val in enumerate(snr_arr):
                        ...
                        while True:
                            ...
                            if ber_reroll_pos:
                                ...
                                for usr_idx, user_pos_tup in enumerate(usr_pos_tup):
                                    ...
                                    my_mcnc_array = copy.deepcopy(my_array)
                                    my_mcnc_array.update_n_users(n_users=1)
                                    my_mcnc_array.set_precoding_matrix(
                                        channel_mat_fd=my_miso_chan.get_channel_mat_fd(),
                                        mr_precoding=mr_precoding,
                                        zf_precoding=zf_precoding)
                                ...
                                for usr_idx, usr_pos in enumerate(usr_pos_tup):
                                    ...
                                    ak_hk_vk_agc_nfft = np.ones(my_mod.n_fft, dtype=np.complex128)
                                    ak_hk_vk_agc_nfft[-(n_sub_carr // 2):] = ak_hk_vk_agc_avg_vec[0:n_sub_carr // 2]
                                    ak_hk_vk_agc_nfft[1:(n_sub_carr // 2) + 1] = ak_hk_vk_agc_avg_vec[
                                                                                 n_sub_carr // 2:]
                                    ak_hk_vk_agc_nfft_lst.append(ak_hk_vk_agc_nfft)
                            ...
                            if any(np.logical_and((n_err[:, 0] < n_err_min), (bits_sent[:, 0] < bits_sent_max))):
                                ...
                                clean_ofdm_symbol = my_array.transmit(tx_bits, out_domain_fd=True,
                                                                      return_both=False,
                                                                      skip_dist=True)
                                    ...
                                    clean_rx_ofdm_symbol = my_noise.process(clean_rx_ofdm_symbol,
                                                                            avg_sample_pow=my_mod.avg_symbol_power *
                                                                                           hk_vk_noise_scaler_lst[
                                                                                               usr_idx],
                                                                            disp_data=False)
                                    clean_rx_ofdm_symbol = np.divide(clean_rx_ofdm_symbol,
                                                                     hk_vk_agc_nfft_lst[usr_idx])
                        ...
                        for usr_idx in range(n_users):
                            data_lst.append(n_err[usr_idx, :])
                            data_lst.append(bits_sent[usr_idx, :])
                    ...
                    for usr_idx in range(n_users):
                        ax1.plot(ebn0_arr, bers_per_usr[usr_idx][0, :], label="No distortion",
                    ...
                    filename_str = "proc_%d_ber_vs_ebn0_mu_%s_mcnc_%s_nant%d_ibo%d_ebn0_min%d_max%d_step%1.2f_niter%s_angles%s_distances%s" % (
                        proc_idx, precoding_str, my_miso_chan, n_ant_val, ibo_val_db, min(ebn0_arr), max(ebn0_arr),
                        ebn0_arr[1] - ebn0_arr[0],
                        '_'.join([str(val) for val in mcnc_n_iter_lst[1:]]),
                        '_'.join([str(val) for val in usr_angles]),
