    ...
    if eps_val in sel_eps_lst:
        for ite_idx, cnc_iter_val in enumerate(cnc_n_iter_lst):
            ...
            if cnc_iter_val in sel_cnc_ite_lst:
                ax1.plot(ebno_lst[0], cnc_los_bers_per_eps[eps_idx][ite_idx + 1], "-", color=CB_color_cycle[color_idx],
                         marker=marker_lst[marker_idx], fillstyle='none')
                ax1.plot(ebno_lst[0], mcnc_los_bers_per_eps[eps_idx][ite_idx + 1], "--",
...
for eps_idx, eps_val in enumerate(sel_eps_lst):
    if eps_val in sel_eps_lst:
        marker_leg_lst.append(
            mlines.Line2D([0], [0], linestyle='none', marker=marker_lst[marker_idx], fillstyle="none", color='k',
                          label=eps_val))
        marker_idx += 1
...
for eps_idx, eps_val in enumerate(csi_eps_lst):
    if eps_val in sel_eps_lst:
        for ite_idx, cnc_iter_val in enumerate(cnc_n_iter_lst):
            if cnc_iter_val in sel_cnc_ite_lst:
                ax2.plot(cnc_los_bers_per_eps[eps_idx][1], cnc_los_bers_per_eps[eps_idx][ite_idx + 1], "-",
                         color=CB_color_cycle[color_idx])
                ax2.plot(mcnc_los_bers_per_eps[eps_idx][1], mcnc_los_bers_per_eps[eps_idx][ite_idx + 1], "--",
                         color=CB_color_cycle[color_idx])
        color_idx += 1
...
ax2.legend(handles=[cnc_leg, mcnc_leg, no_ber_gain], loc="lower center", framealpha=0.9, bbox_to_anchor=(0.62, 0.0))
