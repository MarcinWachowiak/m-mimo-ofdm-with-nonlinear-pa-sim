...
for usr_idx, usr_ber in enumerate(ber_per_dist_per_usr):
    ax1.plot(cnc_ebn0_arr, usr_ber[0], color=CB_color_cycle[0], marker=usr_marker_lst[usr_idx], linestyle='-', fillstyle='none')
    color_idx = 1
    for ite_idx, iter_val in enumerate(cnc_n_iter_lst):
        if iter_val in sel_cnc_iter_val:
            ax1.plot(cnc_ebn0_arr, usr_ber[ite_idx+1], color=CB_color_cycle[color_idx], marker=usr_marker_lst[usr_idx], linestyle='-', fillstyle='none')
            ax1.plot(cnc_ebn0_arr, mu_mcnc_ber_per_usr[usr_idx][ite_idx+1], color=CB_color_cycle[color_idx], marker=usr_marker_lst[usr_idx], linestyle='--', fillstyle='none')
            color_idx += 1
...
leg2 = plt.legend(handles=[usr_1_leg, usr_2_leg], title="User:", loc="lower left", framealpha=0.9, bbox_to_anchor=(0.305, 0.0))
...
plt.savefig("../figs/%s.pdf" % "ber_vs_ebn0_mu_%s_cnc_mcnc_%s_nant%d_ibo%d_ebn0_min%d_max%d_step%1.2f_niter%s_angles%s_distances%s" % (
                        precoding_str, my_miso_chan, n_ant_val, ibo_val_db, ebn0_min, ebn0_max, ebn0_step,
