...
plot_settings.set_latex_plot_style(use_tex=False, fig_width_in=5)
...
for usr_idx, usr_ber in enumerate(ber_per_dist_per_usr):
    ax1.plot(cnc_ebn0_arr, usr_ber[0], color=CB_color_cycle[0], marker=usr_marker_lst[usr_idx],
             fillstyle='none')
    color_idx = 1
    for ite_idx, iter_val in enumerate(cnc_n_iter_lst):
        if iter_val in sel_cnc_iter_val:
            ax1.plot(cnc_ebn0_arr, usr_ber[ite_idx+1], color=CB_color_cycle[color_idx], marker=usr_marker_lst[usr_idx], fillstyle='none')
            color_idx += 1
...
leg2 = plt.legend(handles=[usr_1_leg, usr_2_leg], title="User:", loc="lower left", framealpha=0.9, bbox_to_anchor=(0.2, 0.004))
plt.gca().add_artist(leg2)

cnc_leg = mlines.Line2D([0], [0], linestyle='-', color='k', label='CNC')
mcnc_leg = mlines.Line2D([0], [0], linestyle='--', color='k', label='MCNC')
ax1.legend(handles=[cnc_leg, mcnc_leg], loc="lower left", framealpha=0.9, bbox_to_anchor=(0.33, 0.004))
...
plt.savefig("../figs/%s.png" % cnc_filename_str, dpi=600, bbox_inches='tight')
plt.show()
# plt.cla()
# plt.close()
