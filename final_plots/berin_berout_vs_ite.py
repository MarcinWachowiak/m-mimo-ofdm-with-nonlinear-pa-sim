...
for idx, cnc_iter_val in enumerate(cnc_n_iter_lst[1:]):
    if cnc_iter_val in sel_cnc_iter_val:
        # ber in vs ber out
        ax1.plot(cnc_ber_per_dist[idx+1][::2], cnc_ber_per_dist[idx+2][::2], "-", color=CB_color_cycle[color_idx])
        # ber in vs ber gain
...
for idx, mcnc_iter_val in enumerate(cnc_n_iter_lst[1:]):
    if mcnc_iter_val in sel_cnc_iter_val:
        # ber in vs ber out
        ax1.plot(mcnc_ber_per_dist[idx+1][::2], mcnc_ber_per_dist[idx+2][::2], "--", color=CB_color_cycle[color_idx])
        # ber in vs ber gain
...
for idx, cnc_iter_val in enumerate(cnc_n_iter_lst[1:]):
    if cnc_iter_val in sel_cnc_iter_val:
        # ber in vs ber out
        ax1.plot(cnc_ber_per_dist[1][::2], cnc_ber_per_dist[idx+2][::2], "-", color=CB_color_cycle[color_idx])
        # ber in vs ber gain
        # ax1.plot(cnc_ber_per_dist[idx + 1][::2], np.array(cnc_ber_per_dist[idx + 1][::2]) / np.array(cnc_ber_per_dist[idx + 2][::2]), "-", color=CB_color_cycle[color_idx])
...
for idx, mcnc_iter_val in enumerate(cnc_n_iter_lst[1:]):
    if mcnc_iter_val in sel_cnc_iter_val:
        # ber in vs ber out
        ax1.plot(mcnc_ber_per_dist[1][::2], mcnc_ber_per_dist[idx+2][::2], "--", color=CB_color_cycle[color_idx])
        # ber in vs ber gain
        # ax1.plot(mcnc_ber_per_dist[idx+1][::2], np.array(mcnc_ber_per_dist[idx+1][::2]) / np.array(mcnc_ber_per_dist[idx+2][::2]), "--", color=CB_color_cycle[color_idx])
