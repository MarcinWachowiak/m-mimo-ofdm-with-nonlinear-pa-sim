...
for ite_idx, ite_val in enumerate(cnc_n_iter_lst):
    ax1.plot(ibo_arr_cnc, np.array(bers_per_ibo_cnc[0]) / np.array(bers_per_ibo_cnc[ite_idx]), "-")

plot_settings.reset_color_cycle()

for ite_idx, ite_val in enumerate(cnc_n_iter_lst):
    ax1.plot(ibo_arr_cnc, np.array(bers_per_ibo_mcnc[0]) / np.array(bers_per_ibo_mcnc[ite_idx]), "--")
...
filename_str = "ber_vs_ibo_mcnc_cnc_gain_%s_nant%d_ebn0_%d_ibo_min%d_max%d_step%1.2f_niter%s" % (
