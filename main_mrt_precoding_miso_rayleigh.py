...
ax3.grid(True)
plt.savefig("figs/rayleigh_desired_vs_distortion_beampattern_ibo%d_%dant.png" % (my_tx.impairment.ibo_db, np.max(n_ant_vec)),
...
ax6.grid()
plt.tight_layout()
plt.savefig("figs/rayleigh_psd_at_angle_%ddeg_ibo%d_ant%d.png" % (point_idx_psd, my_tx.impairment.ibo_db, np.max(n_ant_vec)),
            dpi=600,
            bbox_inches='tight')
plt.show()
...
ax7.plot(np.array(sorted_clean_rx_at_max_point_freq_arr), to_db(np.array(sorted_clean_psd_at_max_point_arr)), label="Desired", linewidth=1.0)
...
ax7.plot(np.array(sorted_dist_rx_at_max_point_freq_arr), to_db(np.array(sorted_dist_psd_at_max_point_arr)), label="Distorted", linewidth=1.0)
...
ax7.grid()
plt.tight_layout()
plt.savefig("figs/rayleigh_psd_at_angle_%ddeg_ibo%d_ant%d.png" % (precoding_point_idx, my_tx.impairment.ibo_db, np.max(n_ant_vec)),
            dpi=600,
            bbox_inches='tight')
plt.show()
